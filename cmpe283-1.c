...
struct capability_info procabased[21] =
{
	...
	{ 29, "MONITOR exiting"},
	{ 30, "PAUSE exiting"},
	{ 32, "Activate secondary controls"}
};

struct capability_info second_procabased[23] =
{
...
detect_vmx_features(void)
{
	...
	pr_info("procabased Controls MSR: 0x%llx\n",
		(uint64_t)(lo | (uint64_t)hi << 32));
	report_capability(procabased, 21, lo, hi);

	rdmsr(IA32_VMX_PROCBASED_CTLS2, lo, hi);
	pr_info("second_procabased Controls MSR: 0x%llx\n",
		(uint64_t)(lo | (uint64_t)hi << 32));
	report_capability(second_procabased, 23, lo, hi);

	rdmsr(IA32_VMX_EXIT_CTLS, lo, hi);
	pr_info("vm_exit Controls MSR: 0x%llx\n",
		(uint64_t)(lo | (uint64_t)hi << 32));
	report_capability(vm_exit, 11, lo, hi);

	rdmsr(IA32_VMX_ENTRY_CTLS, lo, hi);
	pr_info("vm_entry Controls MSR: 0x%llx\n",
		(uint64_t)(lo | (uint64_t)hi << 32));
